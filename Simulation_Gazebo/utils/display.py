...
def makeAllFigures(data, pos_obs):
    ...
    sentence = 'Final Distance to Goal position {} m'.format(round(min_dist,3))
    min_dist_obj = plt.plot(time,[min_dist for t in time], '--', color = 'red', label = sentence)
    plt.grid(True)
    plt.legend()
    plt.xlabel('Time (s)')
    plt.ylabel('Distance (m)')
    plt.title('Distance to Goal Position')    
    plt.draw()
    plt.savefig('Pictures/TrackSim1_wp.png', dpi=120)
    ...
    plt.draw()
    plt.savefig('Pictures/TrackSim0_oa.png', dpi=120)
    ...
    plt.draw()
    plt.savefig('Pictures/TrackSim0_ca.png', dpi=120)
    ...
    if id_trackers :
        ...
        plt.grid(True)
        sentence = 'Threshold 10 m'
        threshold = plt.plot(time,[10 for t in time], '--', color = 'red', label = sentence)
        plt.legend()
        plt.xlabel('Time (s)')
        plt.ylabel('Distance (m)')
        ...
        plt.savefig('Pictures/TrackSim1_track.png', dpi=120)
